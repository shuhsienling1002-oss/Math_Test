...
st.title("♾️ 國中數學雲端教室 (V28.1 少量練習版)")
...
if mode == "📂 題庫測驗模式":
    if db is None:
        ...
        # 修正：改為隨機 3 題
        if st.sidebar.button("🚀 生成試卷 (隨機3題)"):
            if q_count < 3:
                selected_qs = db[unit]
            else:
                selected_qs = random.sample(db[unit], 3)
    ...
    if st.sidebar.button("✨ 生成自訂試卷"):
        ...
        for line in raw_text.strip().split('\n'):
            if "|" in line:
                parts = line.split('|')
                q = parts[0].strip(); ans = parts[1].strip()
    ...
    for i, q in enumerate(st.session_state.quiz):
        ...
        with st.expander(f"第 {i+1} 題: {'✅ 正確' if is_correct else '❌ 錯誤'}"):
            if not is_correct and q.get('svg') != 'none':
                 st.markdown(SVGDrawer.draw(q['svg'], **q.get('params', {})), unsafe_allow_html=True)
            st.write(f"題目: {q['q']}"); st.write(f"您的答案: {u_ans}"); st.write(f"正確答案: {q['ans']}")
            st.info(f"解析: {q['expl']}")
    final_score = int((score / len(st.session_state.quiz)) * 100)
    st.markdown(f"## 🏆 最終得分: {final_score} 分")
    if st.button("🔄 再測驗一次"):
        st.session_state.exam_finished = False
        st.session_state.quiz = []; st.rerun()