...
def create_cloud_database():
    ...
    for _ in range(50):
        ...
        a, b, c = triple
        database["3-2 三角形的外心"].append({"q": f"直角三角形兩股長為 {a}, {b}，其「外接圓半徑」為何？", "options": [str(c/2), str(c), str(a+b), str(c*2)], "ans": str(c/2), "expl": "外心在斜邊中點，半徑為斜邊一半。", "svg": "triangle_circumcenter"})
    for _ in range(50):
    ...
    for _ in range(50):
        ...
        database["4-1 因式分解法"].append({"q": f"解方程式 {t1}{t2} = 0？", "options": [f"{r1} 或 {r2}", f"{-r1} 或 {-r2}", f"{r1} 或 {-r2}", "無解"], "ans": f"{r1} 或 {r2}", "expl": f"解為 {r1} 或 {r2}。", "svg": "roots_line_hidden", "svg_params": {"r1_label": "x₁", "r2_label": "x₂", "r1": r1, "r2": r2}})
    for _ in range(50):
        k = random.randint(2, 9)
        database["4-1 因式分解法"].append({"q": f"解方程式 x² - {k}x = 0？", "options": [f"0 或 {k}", f"{k}", "0", f"1 或 {k}"], "ans": f"0 或 {k}", "expl": "提公因式 x(x-k)=0。", "svg": "roots_0_k", "svg_params": {"k_label": "k", "k": k}})
    for _ in range(50):
        k = random.randint(2, 9)
...
class SVGDrawer:
    ...
    def draw(svg_type, **kwargs):
        ...
        if svg_type == "geometry_sas":
            ...
            return base.format(f'<line x1="10" y1="100" x2="290" y2="100" stroke="black"/><circle cx="{mx(0)}" cy="100" r="5" fill="red"/><circle cx="{mx(k)}" cy="100" r="5" fill="red"/><text x="{mx(k)}" y="130" fill="red" text-anchor="middle">{kl}</text><text x="{mx(0)}" y="130" text-anchor="middle">0</text>')
...
unit = st.sidebar.selectbox("單元選擇", list(data.keys()))
...
if st.session_state.quiz and not st.session_state.exam_finished:
    with st.form("exam_form"):
        ...
        for i, q in enumerate(st.session_state.quiz):
            ...
            ans = st.radio("選項", q['options'], key=f"q_{i}_{time.time()}", label_visibility="collapsed")
            u_ans.append(ans)
            st.divider()
