...
def create_cloud_database():
    ...
    for _ in range(50):
        ...
        database["3-1 證明與推理"].append({"q": f"△ABC 中，∠A={a}°，∠B={b}°，則 ∠C 的外角是多少度？", "options": [str(a+b), str(180-(a+b)), "180", "90"], "ans": str(a+b), "expl": f"外角 = 不相鄰兩內角和：{a} + {b} = {a+b}。", "svg": "none"})
    for _ in range(50):
    ...
    for _ in range(50):
        ...
        database["3-1 證明與推理"].append({"q": f"下列何者是「{shape[0]}」對角線必具備的性質？", "options": [shape[1], "只有一條平分", "無特殊性質", "以上皆非"], "ans": shape[1], "expl": f"{shape[0]}的對角線性質：{shape[1]}。", "svg": "none"})
    for _ in range(50):
    ...
    for _ in range(50):
        ...
        a, b, c = triple
        database["3-2 三角形的外心"].append({"q": f"直角三角形兩股長為 {a}, {b}，其「外接圓半徑」為何？", "options": [str(c/2), str(c), str(a+b), str(c*2)], "ans": str(c/2), "expl": "直角三角形外心在斜邊中點，半徑為斜邊一半。", "svg": "triangle_circumcenter"})
    for _ in range(50):
        database["3-2 三角形的外心"].append({"q": "三角形的外心是哪三條線的交點？", "options": ["三邊中垂線", "三角平分線", "三中線", "三高"], "ans": "三邊中垂線", "expl": "外心是三邊中垂線的交點。", "svg": "none"})

    # ================= 3-3 內心 (獨立 100題) =================
    for _ in range(50):
        deg = random.choice([40, 50, 60, 70, 80])
        database["3-3 三角形的內心"].append({"q": f"I 為內心，若 ∠A = {deg}°，求 ∠BIC？", "options": [str(90 + deg//2), str(180-deg), str(90+deg), str(2*deg)], "ans": str(90 + deg//2), "expl": f"內心角度公式：90 + ∠A/2 = {90 + deg//2}。", "svg": "triangle_incenter", "svg_params": {"a": deg}})
    for _ in range(50):
        database["3-3 三角形的內心"].append({"q": "三角形的內心是指哪三條線的交點？", "options": ["三角平分線", "三中垂線", "三中線", "三高"], "ans": "三角平分線", "expl": "內心是三內角平分線的交點。", "svg": "none"})

    # ================= 3-4 重心 (獨立 100題) =================
    for _ in range(50):
        m = random.randint(6, 30) * 3
        database["3-4 三角形的重心"].append({"q": f"若中線 AD 長為 {m}，G 為重心，求 AG？", "options": [str(int(m*2/3)), str(m), str(int(m/3)), str(int(m/2))], "ans": str(int(m*2/3)), "expl": f"重心性質：AG = 2/3 AD = {int(m*2/3)}。", "svg": "triangle_centroid", "svg_params": {"m": m}})
    for _ in range(50):
        area = random.choice([12, 24, 30, 36, 60, 72])
        database["3-4 三角形的重心"].append({"q": f"△ABC 面積為 {area}，G 為重心，則 △GAB 面積為何？", "options": [str(int(area/3)), str(int(area/6)), str(int(area/2)), str(area)], "ans": str(int(area/3)), "expl": f"重心與頂點連線將三角形面積三等分：{area}/3 = {int(area/3)}。", "svg": "none"})
    ...
    for _ in range(50):
        ...
        database["4-1 因式分解法"].append({"q": f"解方程式 {term1}{term2} = 0？", "options": [f"{r1} 或 {r2}", f"{-r1} 或 {-r2}", f"{r1} 或 {-r2}", "無解"], "ans": f"{r1} 或 {r2}", "expl": f"解為 {r1} 或 {r2}。", "svg": "roots_line_hidden", "svg_params": {"r1_label": "x₁", "r2_label": "x₂", "r1": r1, "r2": r2}})
    for _ in range(50):
        k = random.randint(2, 9)
        database["4-1 因式分解法"].append({"q": f"解方程式 x² - {k}x = 0？", "options": [f"0 或 {k}", f"{k}", "0", f"1 或 {k}"], "ans": f"0 或 {k}", "expl": f"提公因式：x(x-{k})=0 => x=0 或 {k}。", "svg": "roots_0_k", "svg_params": {"k_label": "k", "k": k}})
    for _ in range(50):
        k = random.randint(2, 9)
        database["4-1 因式分解法"].append({"q": f"解方程式 x² - {k*k} = 0？", "options": [f"±{k}", f"{k}", str(k*k), "無解"], "ans": f"±{k}", "expl": f"x²={k*k} => x=±{k}。", "svg": "none"})
    for _ in range(50):
        k = random.randint(2, 5)
        database["4-1 因式分解法"].append({"q": f"若 x={k} 是方程式 x² + ax + b = 0 的一根，則？", "options": [f"將 {k} 代入方程式等號成立", f"將 -{k} 代入方程式等號成立", "a必為正", "b必為負"], "ans": f"將 {k} 代入方程式等號成立", "expl": "根代入後能使等號成立。", "svg": "none"})
    for _ in range(50):
    ...
    for _ in range(50):
        ...
        database["4-2 配方法與公式解"].append({"q": f"將 x² + {k}x 配成完全平方式，需加上？", "options": [str((k//2)**2), str(k), str(k*2), "1"], "ans": str((k//2)**2), "expl": f"加上 (一次項係數一半) 的平方：({k}/2)² = {(k//2)**2}。", "svg": "area_square_k"})
    for _ in range(50):
        b, c = random.choice([2, 4, 6]), random.randint(1, 3)
        database["4-2 配方法與公式解"].append({"q": f"方程式 x² + {b}x + {c} = 0 的判別式 D 為何？", "options": [str(b*b-4*c), str(b*b+4*c), "0", "1"], "ans": str(b*b-4*c), "expl": f"D = b² - 4ac = {b*b-4*c}。", "svg": "none"})
    for _ in range(50):
        database["4-2 配方法與公式解"].append({"q": "若判別式 D < 0，則其根的性質？", "options": ["無實根", "重根", "兩相異實根", "無法判斷"], "ans": "無實根", "expl": "D < 0 無實數解。", "svg": "none"})
    for _ in range(50):
        database["4-2 配方法與公式解"].append({"q": "一元二次方程式公式解中，分母為何？", "options": ["2a", "a", "4a", "b"], "ans": "2a", "expl": "公式解分母為 2a。", "svg": "none"})

    # ================= 4-3 應用問題 (250題) =================
    for _ in range(50):
        s = random.randint(5, 20)
        database["4-3 應用問題"].append({"q": f"某正方形面積為 {s*s} 平方公尺，求其邊長？", "options": [str(s), str(s*s), str(s*2), "10"], "ans": str(s), "expl": f"邊長 = √{s*s} = {s}。", "svg": "area_square"})
    for _ in range(50):
        t = random.randint(2, 6)
        database["4-3 應用問題"].append({"q": f"h=5t²。物體落下 {5*t*t} 公尺，需時幾秒？", "options": [str(t), str(t*2), "10", "5"], "ans": str(t), "expl": f"5*t² = {5*t*t} => t² = {t*t} => t = {t}。", "svg": "none"})
    for _ in range(50):
        n = random.randint(1, 10)
        database["4-3 應用問題"].append({"q": f"兩連續正整數的乘積為 {n*(n+1)}，求較小的數？", "options": [str(n), str(n+1), str(n-1), "0"], "ans": str(n), "expl": f"n(n+1) = {n*(n+1)} => n={n}。", "svg": "none"})
    for _ in range(50):
        w = random.randint(3, 8)
        database["4-3 應用問題"].append({"q": f"長方形長比寬多 2，面積為 {w*(w+2)}，求寬？", "options": [str(w), str(w+2), str(w-2), "1"], "ans": str(w), "expl": f"x(x+2) = {w*(w+2)} => x = {w}。", "svg": "none"})
    for _ in range(50):
        database["4-3 應用問題"].append({"q": "解應用問題時，若算出長度為 -5，代表？", "options": ["不合 (長度需為正)", "取絕對值", "當作答案", "重算"], "ans": "不合 (長度需為正)", "expl": "幾何長度在應用問題中必須為正數。", "svg": "none"})
