...
def create_cloud_database():
    ...
    for _ in range(50):
        ...
        elif subtype == 2: # 陷阱題 (SSA/AAA)
            bad = random.choice(["SSA", "AAA"])
            add_q("3-1 證明與推理", "concept",
                  f"下列哪一個條件「無法」保證三角形全等？",
                  [bad, "SAS", "ASA", "SSS"],
                  bad, f"{bad} 只能確定相似(AAA)或不確定(SSA)，非全等性質。")
        else: # 中垂線/角平分線觀念
    ...
    for _ in range(50):
        ...
        add_q("3-1 證明與推理", "real",
              ...
              f"{abs(s1-s2)} < x < {s1+s2}", "三角形兩邊和 > 第三邊，兩邊差 < 第三邊。")
    ...
    for _ in range(50):
        ...
        add_q("3-2 三角形的外心", "concept",
              f"「{tri_type[0]}三角形」的外心位置在哪裡？",
              [tri_type[1], "頂點", "不一定", "重心"],
              tri_type[1], f"{tri_type[0]}三角形外心在{tri_type[1]}。")
    ...
    for _ in range(50):
        ...
        if subtype == 1: # 直角半徑
            ...
            add_q("3-2 三角形的外心", "calc",
                  ...
                  str(c/2), "直角三角形外接圓半徑 = 斜邊的一半。", "triangle_circumcenter")
        else: # 座標
    ...
    # [情境題]
    for _ in range(50):
        add_q("3-2 三角形的外心", "real",
              "三個村莊 A, B, C 想要蓋一座共用的水塔，要求到三村莊距離相等，應蓋在？",
              ["△ABC 的外心", "△ABC 的內心", "△ABC 的重心", "AB 線段上"],
              "△ABC 的外心", "外心到三頂點等距離。")
        
        add_q("3-2 三角形的外心", "real",
              "圓形古蹟破裂殘缺，考古學家想找回圓心復原，應該在圓弧上取點做什麼線？",
              ["弦的中垂線", "切線", "角平分線", "中線"],
              "弦的中垂線", "圓心必在弦的中垂線上，兩條中垂線交點即圓心。")
    ...
    # [觀念題]
    for _ in range(50):
        add_q("3-3 三角形的內心", "concept",
              "內心到三角形哪裡的距離相等？",
              ["三邊", "三頂點", "三中點", "外部"],
              "三邊", "內心為內切圓圓心，半徑即為到三邊距離。")
    ...
    for _ in range(50):
        ...
        if subtype == 1: # 角度
            ...
            add_q("3-3 三角形的內心", "calc",
                  f"I 為內心，∠A={deg}°，求 ∠BIC？",
                  [str(90+deg//2), str(180-deg), str(90+deg), str(deg)],
                  str(90+deg//2), "公式：90 + A/2。", "triangle_incenter", {"a": deg})
        else: # 面積反推
            s = random.randint(10, 20)
            r = random.randint(2, 5)
            area = s * r // 2
            add_q("3-3 三角形的內心", "calc",
                  f"三角形周長 {s}，內切圓半徑 {r}，求面積？",
                  [str(area), str(s*r), str(area*2), str(s+r)],
                  str(area), "面積 = rs/2。")

    # [情境題]
    for _ in range(50):
        add_q("3-3 三角形的內心", "real",
              "想要在三角形公園內蓋一個圓形噴水池，且圓面積最大(不超出邊界)，圓心應選？",
              ["內心", "外心", "重心", "頂點"],
              "內心", "內切圓是三角形內部最大的圓。")
    ...
    for _ in range(50):
        ...
        if subtype == 1: # 長度比
            ...
            add_q("3-4 三角形的重心", "calc",
                  f"G 為重心，中線 AD 長為 {m}，求 AG？",
                  [str(m*2//3), str(m//3), str(m), str(m//2)],
                  str(m*2//3), "重心到頂點距離為中線長的 2/3。", "triangle_centroid", {"m": m})
        else: # 面積分割
            area = random.randint(6, 12) * 6
            add_q("3-4 三角形的重心", "calc",
                  f"△ABC 面積 {area}，G 為重心。則 △GAB 面積為？",
                  [str(area//3), str(area//2), str(area//6), str(area)],
                  str(area//3), "重心與頂點連線將面積三等分。")

    # [情境題]
    for _ in range(50):
        add_q("3-4 三角形的重心", "real",
              "童軍課製作三角形木板，想用一根手指頂住木板讓它平衡，手指要放在？",
              ["重心", "內心", "外心", "垂心"],
              "重心", "重心是物體的重量中心。")
    ...
    # [觀念題]
    for _ in range(50):
        add_q("4-1 因式分解法", "concept",
              "若 (x-a)(x-b) = 0，則下列推論何者正確？",
              ["x=a 或 x=b", "x=a 且 x=b", "x=0", "a=b"],
              "x=a 或 x=b", "零積性質。")
    ...
    for _ in range(50):
        ...
        if subtype == 1: # 平方差
            ...
            add_q("4-1 因式分解法", "calc",
                  f"因式分解 x² - {k*k}？",
                  [f"(x+{k})(x-{k})", f"(x-{k})²", f"(x+{k})²", "無法分解"],
                  f"(x+{k})(x-{k})", "平方差公式。")
        else: # 十字交乘
            a, b = random.randint(1, 5), random.randint(1, 5)
            add_q("4-1 因式分解法", "calc",
                  f"因式分解 x² + {a+b}x + {a*b}？",
                  [f"(x+{a})(x+{b})", f"(x-{a})(x-{b})", f"(x+{a})(x-{b})", "無解"],
                  f"(x+{a})(x+{b})", "十字交乘法。")

    # [情境題]
    for _ in range(50):
        area = random.randint(10, 50)
        add_q("4-1 因式分解法", "real",
              f"長方形面積 {area}，長寬皆為整數，請問長寬可能是？(因式分解概念)",
              ["需找出面積的因數", "需找出面積的倍數", "一定是正方形", "無法判斷"],
              "需找出面積的因數", "長 × 寬 = 面積，故長寬為面積的因數。")
    ...
    for _ in range(50):
        ...
        add_q("4-2 配方法與公式解", "calc",
              f"x² + {k}x + □ 配成完全平方式，□ = ？",
              [str((k//2)**2), str(k), str(k*2), "1"],
              str((k//2)**2), "補項公式：(係數/2)²。", "area_square_k")

    # [情境題]
    for _ in range(50):
        add_q("4-2 配方法與公式解", "real",
              "利用公式解求出時間 t = 3 ± √(-5)，這代表什麼物理意義？",
              ["無解(不可能發生)", "有兩個時間點", "時間倒流", "計算錯誤"],
              "無解(不可能發生)", "根號內為負數(無實數解)，代表物體達不到該高度。")
    ...
    for _ in range(50):
        ...
        add_q("4-3 應用問題", "calc",
              f"某數平方比該數大 {n*(n-1)}，求該數(正整數)？",
              [str(n), str(n+1), str(n-1), "0"],
              str(n), f"x² - x = {n*(n-1)}，解得 x={n}。")
    ...
    for _ in range(50):
        ...
        if subtype == 1: # 拋體
            ...
            add_q("4-3 應用問題", "real",
                  f"煙火發射高度 h = 20t - 5t²。在 t={t} 秒時高度為 {20*t - 5*t*t}，求 t？",
                  [str(t), str(t+1), "0", "10"],
                  str(t), "代入公式解方程式。")
        else: # 梯子
            a, b, c = random.choice([(3,4,5), (5,12,13), (8,15,17)])
            add_q("4-3 應用問題", "real",
                  f"梯子長 {c} 公尺，梯腳離牆 {a} 公尺，梯頂高度？",
...
st.title("☁️ 國中數學智能題庫 (V7.2 完整邏輯版)")

if 'quiz' not in st.session_state: st.session_state.quiz = []
if 'exam_finished' not in st.session_state: st.session_state.exam_finished = False

data = create_cloud_database()
st.sidebar.success(f"✅ 題庫生成完畢！(完整變異邏輯)")
