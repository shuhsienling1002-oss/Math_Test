...
# ==========================================
# 1. 核心：雲端題庫製造機 (V7.4 終極完整版 - 邏輯全開 + 圖形修復)
# ==========================================
def create_cloud_database():
    ...
    for _ in range(50):
        ...
        if subtype == 1:
            ...
            add_q("3-1 證明與推理", "concept",
                  ...
                  "必全等", f"{prop} 是五大三角形全等判別性質之一。", "geometry_sas")
        elif subtype == 2:
    ...
    for _ in range(50):
        ...
        add_q("3-1 證明與推理", "real",
              ...
              f"{abs(s1-s2)} < x < {s1+s2}", "三角形兩邊和 > 第三邊。")

    # =================================================================
    # 單元 3-2: 外心 (完整變異邏輯 + 圖形修正)
    # =================================================================
    for _ in range(50):
        add_q("3-2 三角形的外心", "concept", "三角形的外心是哪三條線的交點？", ["中垂線", "角平分線", "中線", "高"], "中垂線", "外心定義。")
    ...
    for _ in range(50):
        ...
        if subtype == 1: 
            ...
            add_q("3-2 三角形的外心", "calc",
                  ...
                  f"({k//2},{k//2})", "直角三角形外心為斜邊中點。")

    for _ in range(50):
        add_q("3-2 三角形的外心", "real", "三個村莊 A, B, C 想要蓋一座共用的水塔，要求到三村莊距離相等，應蓋在？", ["△ABC 的外心", "△ABC 的內心", "△ABC 的重心", "AB 線段上"], "△ABC 的外心", "外心到三頂點等距離。")
    ...
    # =================================================================
    for _ in range(50):
        add_q("3-3 三角形的內心", "concept", "內心到三角形哪裡的距離相等？", ["三邊", "三頂點", "三中點", "外部"], "三邊", "內心為內切圓圓心。")
    ...
    for _ in range(50):
        add_q("3-3 三角形的內心", "real", "想要在三角形公園內蓋一個圓形噴水池，且圓面積最大，圓心應選？", ["內心", "外心", "重心", "頂點"], "內心", "內切圓是三角形內部最大的圓。")

    # =================================================================
    # 單元 3-4: 重心 (完整變異邏輯)
    # =================================================================
    for _ in range(50):
        add_q("3-4 三角形的重心", "concept", "三角形的重心是哪三條線的交點？", ["中線", "中垂線", "角平分線", "高"], "中線", "重心定義。")
    ...
    for _ in range(50):
        add_q("3-4 三角形的重心", "real", "童軍課製作三角形木板，想用一根手指頂住木板讓它平衡，手指要放在？", ["重心", "內心", "外心", "垂心"], "重心", "重心是物體的重量中心。")
    ...
    for _ in range(50):
        ...
        if subtype == 1:
            ...
            add_q("4-1 因式分解法", "calc", f"因式分解 x² - {k*k}？", [f"(x+{k})(x-{k})", f"(x-{k})²", f"(x+{k})²", "無法分解"], f"(x+{k})(x-{k})", "平方差公式。")
        else:
    ...
    for _ in range(50):
        ...
        add_q("4-1 因式分解法", "real", f"長方形面積 {area}，長寬皆為整數，請問長寬可能是？", ["需找出面積的因數", "需找出面積的倍數", "一定是正方形", "無法判斷"], "需找出面積的因數", "長 × 寬 = 面積。")

    # =================================================================
    # 單元 4-2: 配方法 (完整變異邏輯)
    # =================================================================
    for _ in range(50):
        add_q("4-2 配方法與公式解", "concept", "一元二次方程式判別式 D < 0 代表？", ["無實根", "重根", "兩相異實根", "有三個根"], "無實根", "D < 0 圖形與 x 軸無交點。")
    ...
    for _ in range(50):
        add_q("4-2 配方法與公式解", "real", "利用公式解求出時間 t = 3 ± √(-5)，這代表什麼物理意義？", ["無解(不可能發生)", "有兩個時間點", "時間倒流", "計算錯誤"], "無解(不可能發生)", "根號內為負數代表無實數解。")
    ...
    for _ in range(50):
        ...
        if subtype == 1:
            ...
            add_q("4-3 應用問題", "real", f"煙火發射高度 h = 20t - 5t²。在 t={t} 秒時高度為 {20*t - 5*t*t}，求 t？", [str(t), str(t+1), "0", "10"], str(t), "代入公式解方程式。")
        else:
            a, b, c = random.choice([(3,4,5), (5,12,13), (8,15,17)])
            add_q("4-3 應用問題", "real", f"梯子長 {c} 公尺，梯腳離牆 {a} 公尺，梯頂高度？", [str(b), str(c), str(a+b), str(c-a)], str(b), f"畢氏定理。")

    return database

# ==========================================
# 2. 視覺繪圖引擎 (V7.4 完整版 - 含直角外心修正)
# ==========================================
class SVGDrawer:
    @staticmethod
    def draw(svg_type, **kwargs):
        ...
        # 🔥 V7.4 專用：直角三角形的外心 (斜邊中點)
        elif svg_type == "right_triangle_circumcenter":
            return base.format(
                '<circle cx="150" cy="100" r="80" fill="none" stroke="green"/>'
                '<path d="M90,40 L90,160 L210,160 Z" fill="none" stroke="black" stroke-width="2"/>'
                '<circle cx="150" cy="100" r="5" fill="red"/>'
                '<text x="160" y="95" fill="red" font-weight="bold">O (斜邊中點)</text>'
            )
...
st.title("☁️ 國中數學智能題庫 (V7.4 終極完整版)")

if 'quiz' not in st.session_state: st.session_state.quiz = []
if 'exam_finished' not in st.session_state: st.session_state.exam_finished = False

data = create_cloud_database()
st.sidebar.success(f"✅ 題庫生成完畢！(完整變異+圖形修復)")
