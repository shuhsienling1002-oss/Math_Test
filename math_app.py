...
# ==========================================
# 1. 核心：雲端題庫製造機 (嚴格執行 1250 題，單元獨立化)
# ==========================================
@st.cache_data
def create_cloud_database():
    ...
    for _ in range(50):
        ...
        database["4-1 因式分解法"].append({"q": f"若 x={k} 是方程式的根，則？", "options": [f"代入 {k} 等號成立", "代入 -k 成立", "a必正", "b必負"], "ans": f"代入 {k} 等號成立", "expl": "根的定義。", "svg": "none"})
    for _ in range(50):
        k = random.randint(1, 9)
        database["4-1 因式分解法"].append({"q": f"方程式 (x-{k})² = 0 有幾個解？", "options": ["1個 (重根)", "2個相異解", "無解", "無限多"], "ans": "1個 (重根)", "expl": "重根為一個解。", "svg": "none"})
    ...
    for _ in range(50):
        database["4-2 配方法與公式解"].append({"q": "若 D < 0，則根的性質？", "options": ["無實根", "重根", "相異實根", "無法判斷"], "ans": "無實根", "expl": "判別式性質。", "svg": "none"})
    for _ in range(50):
        database["4-2 配方法與公式解"].append({"q": "公式解的分母為何？", "options": ["2a", "a", "4a", "b"], "ans": "2a", "expl": "公式解公式。", "svg": "none"})

    # ================= 4-3 應用問題 (5變化 x 50 = 250題) =================
    for _ in range(50):
        s = random.randint(5, 20)
        database["4-3 應用問題"].append({"q": f"正方形面積為 {s*s}，求邊長？", "options": [str(s), str(s*s), str(s*2), "10"], "ans": str(s), "expl": "面積開根號。", "svg": "area_square"})
    for _ in range(50):
        t = random.randint(2, 6)
        database["4-3 應用問題"].append({"q": f"h=5t²。落下 {5*t*t} 公尺，需幾秒？", "options": [str(t), str(t*2), "10", "5"], "ans": str(t), "expl": "代入公式解 t。", "svg": "none"})
    for _ in range(50):
        n = random.randint(1, 10)
        database["4-3 應用問題"].append({"q": f"兩連續正整數乘積 {n*(n+1)}，較小數？", "options": [str(n), str(n+1), str(n-1), "0"], "ans": str(n), "expl": "驗算乘積。", "svg": "none"})
    for _ in range(50):
        w = random.randint(3, 8)
        database["4-3 應用問題"].append({"q": f"長比寬多 2，面積 {w*(w+2)}，寬？", "options": [str(w), str(w+2), str(w-2), "1"], "ans": str(w), "expl": "寬為 w。", "svg": "none"})
    for _ in range(50):
        database["4-3 應用問題"].append({"q": "應用題算出長度 -5 代表？", "options": ["不合", "正確", "取絕對值", "重算"], "ans": "不合", "expl": "長度必為正。", "svg": "none"})
...
class SVGDrawer:
    ...
    def draw(svg_type, **kwargs):
        ...
        if svg_type == "geometry_sas":
            ...
            return base.format(f'<line x1="10" y1="100" x2="290" y2="100" stroke="black"/><circle cx="{mx(r1)}" cy="100" r="5" fill="red"/><text x="{mx(r1)}" y="130" fill="red">{l1}</text><circle cx="{mx(r2)}" cy="100" r="5" fill="red"/><text x="{mx(r2)}" y="130" fill="red">{l2}</text>')
        elif svg_type == "roots_0_k":
            k = kwargs.get('k', 0)
            kl = kwargs.get('k_label', 'k')
            return base.format(f'<line x1="10" y1="100" x2="290" y2="100" stroke="black"/><circle cx="{mx(0)}" cy="100" r="5" fill="red"/><circle cx="{mx(k)}" cy="100" r="5" fill="red"/><text x="{mx(k)}" y="130" fill="red">{kl}</text>')
        elif svg_type == "area_square":
            return base.format('<rect x="100" y="50" width="100" height="100" fill="#e3f2fd" stroke="black"/><text x="150" y="105" text-anchor="middle">Area</text>')
