...
def create_cloud_database():
    ...
    for _ in range(50):
        database["3-3 三角形的內心"].append({
            "q": "三角形的內心到三邊的距離關係為何？",
            "options": ["等距離", "不相等", "到頂點距離的一半", "兩倍距離"],
            "ans": "等距離",
            "expl": "內心是三內角平分線的交點，到三角形三邊等距離。",
            "svg": "none"
        })

    # ================= 3-4 重心 (2種變化 x 50 = 100題) =================
    for _ in range(50):
    ...
    for _ in range(50):
        ...
        database["3-4 三角形的重心"].append({
            ...
            "ans": str(int(area/3)),
            "expl": f"重心與三頂點連線將三角形面積三等分：{area}/3 = {int(area/3)}。",
            "svg": "none"
        })

    # ================= 4-1 因式分解法 (5種變化 x 50 = 250題) =================
    for _ in range(50):
    ...
    for _ in range(50):
        ...
        database["4-3 應用問題"].append({
            ...
            "ans": str(n),
            "expl": f"設數為 n, n+1，則 n(n+1) = {n*(n+1)}。",
            "svg": "none"
        })
    for _ in range(50):
...
if st.sidebar.button("🚀 生成試卷 (10題)"):
    ...
    # 使用 random.sample 確保抽出 10 題絕對不重複
