...
class QuestionFactory:
    ...
    def parse_custom_text(raw_text):
        ...
        for line in lines:
            ...
            try:
                ...
                ans = parts[1].strip()
                if len(parts) > 2:
                    wrongs = [x.strip() for x in parts[2].split(',')]
                    options = wrongs + [ans]
                else:
                    if ans.isdigit(): options = MathUtils.get_distractors(ans) + [ans]
                    else: options = ["選項A", "選項B", "選項C", ans]
                random.shuffle(options)
                questions.append({"q": q_text, "options": options, "ans": ans, "expl": "自訂題目解析", "svg": "none"})
            except: continue
        return questions
    ...
    def gen_3_1(q_type):
        if q_type == "concept":
            ...
            if subtype == "congruence":
                ...
                return {"q": f"若兩個三角形滿足「{prop}」對應相等，則它們？", "options": ["必全等", "相似", "面積相等", "不一定"], "ans": "必全等", "expl": f"{prop} 是全等性質。", "svg": "geometry_sas"}
            elif subtype == "inequality":
                return {"q": "在一個三角形中，若 ∠A > ∠B，則對邊關係為何？", "options": ["BC > AC", "AC > BC", "BC = AC", "無法判斷"], "ans": "BC > AC", "expl": "大角對大邊性質。", "svg": "none"}
            elif subtype == "logic":
                return {"q": "「若 P 則 Q」中，Q 稱為？", "options": ["結論", "題設", "逆敘述", "公理"], "ans": "結論", "expl": "邏輯定義。", "svg": "none"}
            else:
                bad = random.choice(["SSA", "AAA"])
                return {"q": f"下列何者「無法」保證全等？", "options": [bad, "SAS", "ASA", "SSS"], "ans": bad, "expl": f"{bad} 不保證全等。", "svg": "none"}
        elif q_type == "calc":
            ...
            if subtype == "angle_sum":
                ...
                sides = [5, 6, 7]; random.shuffle(sides); a, b, c = sides; min_side = min(a, b, c); ans_map = {a: "∠A", b: "∠B", c: "∠C"}; ans = ans_map[min_side]
                return {"q": f"△ABC 中，AB={c}, BC={a}, AC={b}，請問哪一個角最小？", "options": ["∠A", "∠B", "∠C", "一樣大"], "ans": ans, "expl": "小邊對小角。", "svg": "none"}
            else:
            ...
            scenarios = [{"q": "兩根吸管長 {s1}, {s2}，第三邊 x 的範圍？", "type": "sticks"}, {"q": "地板鋪滿正六邊形磁磚，接點有 3 塊，利用了？", "type": "tiles"}, {"q": "小明走捷徑穿越草地(三角形兩邊)，而不走直角轉彎，是因為？", "type": "shortcut"}]
            s = random.choice(scenarios)
            if s["type"] == "sticks":
                s1, s2 = random.randint(5, 20), random.randint(5, 20); min_x, max_x = abs(s1 - s2), s1 + s2
    ...
    def gen_3_2(q_type):
        if q_type == "concept":
            ...
            return {"q": f"「{tri_type[0]}三角形」的外心位置在哪裡？", "options": [tri_type[1], "頂點", "重心", "不一定"], "ans": tri_type[1], "expl": "外心位置性質。", "svg": "triangle_circumcenter"}
        elif q_type == "calc":
            subtype = random.choice(["right_R", "reverse_R", "coord_O"])
            if subtype == "right_R":
                c = random.randint(5, 30) * 2; r = c // 2; opts = MathUtils.get_distractors(r) + [str(r)]; random.shuffle(opts)
                return {"q": f"直角三角形斜邊長 {c}，外接圓半徑 R？", "options": opts, "ans": str(r), "expl": "直角外心在斜邊中點。", "svg": "right_triangle_circumcenter"}
            elif subtype == "reverse_R":
                r = random.randint(3, 15); ans = r * 2; opts = MathUtils.get_distractors(ans) + [str(ans)]; random.shuffle(opts)
                return {"q": f"直角三角形的外接圓半徑為 {r}，求斜邊長？", "options": opts, "ans": str(ans), "expl": "斜邊 = 2R。", "svg": "right_triangle_circumcenter"}
            else:
                k = random.randint(2, 8) * 2; ans = f"({k//2},{k//2})"; opts = MathUtils.get_distractors(ans, "coord") + [ans]; random.shuffle(opts)
                return {"q": f"A(0,{k}), B({k},0), O(0,0)，求外心座標？", "options": opts, "ans": ans, "expl": "直角三角形外心為斜邊中點。", "svg": "coord_triangle", "params": {"k": k}}
        else:
            scenarios = [{"q": "三村莊 A, B, C 想蓋共用水塔(到三點等距)，選址？", "type": "tower"}, {"q": "考古學家挖到圓盤碎片，想復原圓盤大小，應找？", "type": "plate"}, {"q": "要在圓形廣場周圍裝設三個監視器，監視器連線構成銳角三角形，監控中心(外心)會在？", "type": "camera"}]
            s = random.choice(scenarios)
            if s["type"] == "tower": return {"q": s["q"], "options": ["外心", "內心", "重心", "垂心"], "ans": "外心", "expl": "外心到三頂點等距。", "svg": "triangle_circumcenter"}
    ...
    def gen_3_3(q_type):
        if q_type == "concept": return {"q": "內心到三角形哪裡的距離相等？", "options": ["三邊", "三頂點", "三中點", "外部"], "ans": "三邊", "expl": "內切圓性質。", "svg": "triangle_incenter_concept"}
            ...
            if subtype == "angle":
                deg = random.randint(30, 100); deg += 1 if deg%2!=0 else 0; ans = 90 + deg // 2; opts = MathUtils.get_distractors(ans) + [str(ans)]; random.shuffle(opts)
                ...
                k = random.randint(1,4); a,b,c = 3*k, 4*k, 5*k; ans = (a+b-c)//2; opts = MathUtils.get_distractors(ans) + [str(ans)]; random.shuffle(opts)
                return {"q": f"直角三角形三邊 {a},{b},{c}，內切圓半徑？", "options": opts, "ans": str(ans), "expl": "(股+股-斜)/2。", "svg": "right_triangle_incenter", "params": {"a":a,"b":b,"c":c}}
            elif subtype == "area":
                s = random.randint(10, 30); r = random.randint(2, 6); area = s * r // 2; opts = MathUtils.get_distractors(area) + [str(area)]; random.shuffle(opts)
                return {"q": f"周長 {s}，內切圓半徑 {r}，求三角形面積？", "options": opts, "ans": str(area), "expl": "面積 = rs/2。", "svg": "triangle_incenter_concept"}
            else:
                ans_a = random.randint(40, 100); ans_a += 1 if ans_a%2!=0 else 0; bic = 90 + ans_a // 2; opts = MathUtils.get_distractors(ans_a) + [str(ans_a)]; random.shuffle(opts)
                return {"q": f"I 為內心，若 ∠BIC={bic}°，則 ∠A 是幾度？", "options": opts, "ans": str(ans_a), "expl": "(BIC-90)*2。", "svg": "triangle_incenter_angle", "params": {"a": "?"}}
        else:
            scenarios = [{"q": "公園內蓋最大圓形噴水池，圓心選？", "type": "fountain"}, {"q": "物流中心要蓋在三條公路之間且等距，應選？", "type": "roads"}, {"q": "想要做一個三角形的內切圓時鐘，圓心應如何找？", "type": "clock"}]
            s = random.choice(scenarios)
            if s["type"] == "fountain": return {"q": s["q"], "options": ["內心", "外心", "重心", "頂點"], "ans": "內心", "expl": "內切圓。", "svg": "triangle_incenter_concept"}
    ...
    def gen_3_4(q_type):
        if q_type == "concept": return {"q": "重心是哪三條線的交點？", "options": ["中線", "中垂線", "角平分線", "高"], "ans": "中線", "expl": "重心定義。", "svg": "triangle_centroid"}
            ...
            if subtype == "len_ratio":
                m = random.randint(3, 15) * 3; ans = m * 2 // 3; opts = MathUtils.get_distractors(ans) + [str(ans)]; random.shuffle(opts)
                ...
                x1,y1=random.randint(0,6)*3,random.randint(0,6)*3; x2,y2=random.randint(0,6)*3,random.randint(0,6)*3; x3,y3=random.randint(0,6)*3,random.randint(0,6)*3
                gx,gy=(x1+x2+x3)//3, (y1+y2+y3)//3; ans = f"({gx},{gy})"; opts = MathUtils.get_distractors(ans, "coord") + [ans]; random.shuffle(opts)
                return {"q": f"A({x1},{y1}), B({x2},{y2}), C({x3},{y3})，求重心 G？", "options": opts, "ans": ans, "expl": "三點座標相加除以 3。", "svg": "none"}
            else:
                total = random.randint(2, 12) * 6; ans = total // 6; opts = MathUtils.get_distractors(ans) + [str(ans)]; random.shuffle(opts)
                return {"q": f"△ABC 面積 {total}，G 為重心，△GAB 內的中線分割出的最小三角形面積？", "options": opts, "ans": str(ans), "expl": "重心將面積六等分。", "svg": "triangle_centroid"}
        else:
            scenarios = [{"q": "手指頂住三角形木板平衡，支點是？", "type": "balance"}, {"q": "要用一條繩子吊起一塊三角形招牌並保持水平，繩子應綁在？", "type": "hanging"}, {"q": "把三角形披薩平分給 6 個人，切點應選？", "type": "pizza"}]
            s = random.choice(scenarios)
            if s["type"] == "balance": return {"q": s["q"], "options": ["重心", "內心", "外心", "垂心"], "ans": "重心", "expl": "物理重心。", "svg": "triangle_centroid"}
    ...
    def gen_4_1(q_type):
        if q_type == "concept": return {"q": "若 ab=0，則？", "options": ["a=0 或 b=0", "a=0 且 b=0", "a=b", "無法判斷"], "ans": "a=0 或 b=0", "expl": "零積性質。", "svg": "none"}
            ...
            if subtype == "diff_sq":
                k = random.randint(2, 9); ans = f"(x+{k})(x-{k})"; opts = [ans, f"(x-{k})²", f"(x+{k})²", f"x(x-{k})"]; random.shuffle(opts)
                return {"q": f"因式分解 x² - {k*k}？", "options": opts, "ans": ans, "expl": "平方差公式。", "svg": "diff_squares", "params": {"k": k}}
            elif subtype == "common_factor":
                a = random.randint(2, 5); b = random.randint(2, 5); ans = f"x({a}x+{b})"
                return {"q": f"因式分解 {a}x² + {b}x？", "options": [ans, f"x({a}x-{b})", f"x²({a}+{b})", f"({a}x+1)({b}x)"], "ans": ans, "expl": "提公因式 x。", "svg": "none"}
            elif subtype == "perfect_sq_k":
                b = random.randint(2, 10) * 2; ans = (b // 2) ** 2; opts = MathUtils.get_distractors(ans) + [str(ans)]; random.shuffle(opts)
                return {"q": f"若 x² + {b}x + k 是完全平方式，求 k？", "options": opts, "ans": str(ans), "expl": "一次項係數一半的平方。", "svg": "area_square_k"}
            else:
                p = random.randint(1,5); q_val = random.randint(1,5); b = p+q_val; c = p*q_val; ans = f"(x+{p})(x+{q_val})"; opts = [ans, f"(x-{p})(x-{q_val})", f"(x+{p})(x-{q_val})", f"(x+{b})(x+1)"]; random.shuffle(opts)
                return {"q": f"因式分解 x² + {b}x + {c}？", "options": opts, "ans": ans, "expl": "十字交乘。", "svg": "none"}
        else:
            scenarios = [{"q": "長方形面積 {area}，長寬為整數，長寬關係？", "type": "rect"}, {"q": "全班 x 人，剛好排成 x² + 5x + 6 的隊形，若 x=10，分兩隊？", "type": "group"}, {"q": "某數平方減 9 可以被分解為 (x+3)(x-3)，這是利用？", "type": "diff"}]
            s = random.choice(scenarios)
            if s["type"] == "rect":
    ...
    def gen_4_2(q_type):
        if q_type == "concept":
            ...
            if case == "pos": return {"q": "判別式 D > 0 代表？", "options": ["兩相異實根", "重根", "無實根", "無限多解"], "ans": "兩相異實根", "expl": "與 x 軸有兩個交點。", "svg": "none"}
            elif case == "zero": return {"q": "判別式 D = 0 代表？", "options": ["重根(兩相等實根)", "兩相異實根", "無實根", "無解"], "ans": "重根(兩相等實根)", "expl": "與 x 軸相切。", "svg": "none"}
            else: return {"q": "判別式 D < 0 代表？", "options": ["無實根", "重根", "兩相異實根", "無限多解"], "ans": "無實根", "expl": "與 x 軸無交點。", "svg": "parabola_d_neg"}
        elif q_type == "calc":
            subtype = random.choice(["complete_sq", "sum_roots", "discriminant", "formula"])
            if subtype == "discriminant":
                d_case = random.choice(["pos", "zero", "neg"])
                if d_case == "pos": eq = "x² + 5x + 4 = 0"; ans = "兩相異實根"; expl = "D = 25-16 > 0"
                elif d_case == "zero": eq = "x² + 4x + 4 = 0"; ans = "重根"; expl = "D = 16-16 = 0"
                else: eq = "x² + x + 5 = 0"; ans = "無實根"; expl = "D = 1-20 < 0"
                return {"q": f"方程式 {eq} 的根的性質？", "options": ["兩相異實根", "重根", "無實根", "無限多解"], "ans": ans, "expl": expl, "svg": "none"}
            elif subtype == "complete_sq":
                k = random.randint(2, 10) * 2; ans = (k//2)**2; opts = MathUtils.get_distractors(ans) + [str(ans)]; random.shuffle(opts)
                return {"q": f"x² + {k}x + □ 配成完全平方，□ = ？", "options": opts, "ans": str(ans), "expl": "一半的平方。", "svg": "area_square_k"}
            elif subtype == "sum_roots":
                r1, r2 = random.randint(-5, 5), random.randint(-5, 5); b = -(r1 + r2); c = r1 * r2; ans = r1 + r2
                eq = f"x² + {b}x + {c} = 0" if b >= 0 else f"x² - {-b}x + {c} = 0"
                opts = MathUtils.get_distractors(ans) + [str(ans)]; random.shuffle(opts)
                return {"q": f"方程式 {eq} 的兩根之和為何？", "options": opts, "ans": str(ans), "expl": "兩根和 = -b/a。", "svg": "none"}
            else:
                ans = "x = (-b ± √D) / 2a"; opts = [ans, "x = -b / 2a", "x = (-b ± √D) / a", "x = b ± √D"]; random.shuffle(opts)
                return {"q": "一元二次方程式公式解為何？", "options": opts, "ans": ans, "expl": "公式解。", "svg": "none"}
        else:
            scenarios = [{"q": "長方形花圃長20寬10，中間開闢等寬道路，剩餘面積144，求路寬？", "type": "path"}, {"q": "計算物體落地時間 t 得到虛數，代表？", "type": "imaginary"}, {"q": "長方形剪掉正方形後與原形相似(黃金比例)，長寬比 x 的方程式？", "type": "golden"}, {"q": "彈簧掛重物後長度 y = x² + 2x + 5，若 y=4 (比原長還短)，求 x？", "type": "spring"}]
            s = random.choice(scenarios)
            if s["type"] == "path": return {"q": s["q"], "options": ["2", "4", "5", "8"], "ans": "2", "expl": "(20-x)(10-x)=144，解得 x=2。", "svg": "rect_path"}
            elif s["type"] == "imaginary": return {"q": s["q"], "options": ["物體不落地", "有兩個時間", "計算錯", "時間倒流"], "ans": "物體不落地", "expl": "無實數解。", "svg": "parabola_d_neg"}
    ...
    def gen_4_3(q_type):
        if q_type == "concept": return {"q": "解幾何邊長為負數，應？", "options": ["捨去", "取絕對值", "保留", "重算"], "ans": "捨去", "expl": "長度為正。", "svg": "none"}
            ...
            if subtype == "num_sq":
                n = random.randint(2, 10); val = n*n - n; opts = MathUtils.get_distractors(n) + [str(n)]; random.shuffle(opts)
                return {"q": f"某正數平方減去該數等於 {val}，求該數？", "options": opts, "ans": str(n), "expl": "x^2-x-val=0。", "svg": "none"}
            else:
                h = random.randint(1,5); k = random.randint(5,15); ans = k; opts = MathUtils.get_distractors(ans) + [str(ans)]; random.shuffle(opts)
                return {"q": f"y = -2(x-{h})² + {k} 的最大值？", "options": opts, "ans": str(ans), "expl": "頂點 y 座標。", "svg": "parabola_firework"}
        else:
            scenarios = [{"type": "ladder", "q": "梯子長 {c}，梯腳離牆 {a}，梯頂高度？"}, {"type": "profit", "q": "賣 {p} 元時利潤最大，利潤函數可能是？"}, {"type": "speed", "q": "甲乙兩地距離 {d} km，去程時速 {v1}，回程時速 {v2}，平均速率？"}, {"type": "firework", "q": "煙火 h=30t-5t²，t={t} 時高度？"}, {"type": "tv", "q": "電視長 {a} 吋，寬 {b} 吋，請問這是幾吋電視(對角線)？"}, {"type": "taxi", "q": "計程車起跳 70 元，每公里加 20 元，跑 {x} 公里多少錢？"}]
            s = random.choice(scenarios)
            if s["type"] == "ladder":
                m=random.randint(2,6); n=1; a=m*m-n*n; b=2*m*n; c=m*m+n*n; opts = MathUtils.get_distractors(b) + [str(b)]; random.shuffle(opts)
                return {"q": s["q"].format(c=c, a=a), "options": opts, "ans": str(b), "expl": "畢氏定理。", "svg": "ladder_wall", "params": {"a":a,"b":b,"c":c}}
            elif s["type"] == "profit":
                p = random.randint(10, 50)
                return {"q": s["q"].format(p=p), "options": [f"y=-(x-{p})²+100", f"y=(x-{p})²+100", f"y=x-{p}", "y=x²"], "ans": f"y=-(x-{p})²+100", "expl": "開口向下拋物線頂點。", "svg": "none"}
            elif s["type"] == "speed":
                d = 60; v1 = 20; v2 = 30; ans = 24
                return {"q": s["q"].format(d=d, v1=v1, v2=v2), "options": ["24", "25", "20", "30"], "ans": "24", "expl": "總距離/總時間 = 120/5 = 24。", "svg": "none"}
            elif s["type"] == "firework":
                t=random.randint(2,4); h=30*t-5*t*t; opts = [str(h), "0", "100", "50"]; random.shuffle(opts)
                return {"q": s["q"].format(t=t), "options": opts, "ans": str(h), "expl": "代入。", "svg": "parabola_firework"}
            elif s["type"] == "tv":
                k = random.randint(5, 10); a, b, c = 4*k, 3*k, 5*k; opts = MathUtils.get_distractors(c) + [str(c)]; random.shuffle(opts)
                return {"q": s["q"].format(a=a, b=b), "options": opts, "ans": str(c), "expl": "畢氏定理求對角線。", "svg": "rect_diag", "params": {"a":a, "b":b}}
            elif s["type"] == "taxi":
                x = random.randint(2, 10); ans = 70 + 20 * x; opts = MathUtils.get_distractors(ans) + [str(ans)]; random.shuffle(opts)
                return {"q": s["q"].format(x=x), "options": opts, "ans": str(ans), "expl": "y = 20x + 70。", "svg": "none"}
...
mode = st.sidebar.radio("請選擇出題模式", ["🤖 無限生成模式", "📝 自訂貼上題庫"])
...
台灣最高的山是? | 玉山 | 阿里山, 陽明山, 雪山
因式分解 x²-1 | (x+1)(x-1) | (x+1)², (x-1)², x(x-1)"""
    raw_text = st.sidebar.text_area("在此貼上題目 (一行一題)", default_text, height=200)
    
    if st.sidebar.button("✨ 解析並生成試卷"):
        custom_quiz = QuestionFactory.parse_custom_text(raw_text)
        if custom_quiz:
            st.session_state.quiz = custom_quiz
            st.session_state.exam_finished = False
            st.success(f"成功載入 {len(custom_quiz)} 題！")
            st.rerun()
        else:
            st.sidebar.error("格式錯誤，請檢查分隔線 |")
...
if st.session_state.exam_finished:
    ...
    if st.button("🔄 重來一次", use_container_width=True):
        st.session_state.exam_finished = False
        st.rerun()
